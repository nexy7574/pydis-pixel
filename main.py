...
headers = {
    "User-Agent": "Notice me senpai uwu (https://yourapps.cyou)",
...
def handle_sane_ratelimit(res):
    ...
    if res.status_code == 429:
        log.write(f"[{datetime.utcnow().strftime('%x')} | WARN] Ratelimited for {res.headers['cooldown-reset']} seconds.\n")
        ...
        if int(res.headers["requests-remaining"]) == 0 or int(res.headers["requests-limit"]) == 0:
            log.write(f"[{datetime.utcnow().strftime('%x')} | WARN] Soft ratelimited for {res.headers['requests-reset']} secs.\n")
...
def set_pixel(pixel: Pixel, check: bool = False):
    ...
    if response.status_code == 429:
        set_pixel(pixel)
    log.write(f"[{datetime.utcnow().strftime('%x')} | INFO] Set pixel {repr(pixel)}.")
